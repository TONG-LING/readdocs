...
.. code-block:: text
   ...
   但好样本出来一次之后，下一轮大部分又散回去了，没有带着整批一起变好。
...
.. code-block:: text
   ...
   代理模型在很多轮次里基本选不出来，192 个候选差不多同分，硬选 95 个。
   不是”代理有一点误差”的问题，是很多轮次里代理基本起不到筛选作用。
...
2. fine-tune 存在重置问题
   ...
   - 这样看起来是连续微调，实际上每次都是从同一个起点重新学，没法沿着已经找到的好方向继续走。
...
4. reward 离散化存在风险
   ...
   - 这个从代码上看风险比较大，但还需要结合 candidate pool 原始浮点文件单独验证一下。

为什么我觉得问题主要在继承机制，不全是扩散模型的问题：
...
.. code-block:: text
   ...
   所以更像是：好样本偶尔出现一次，但没有被保留住，也没有吸引更多样本往它那边靠。
   问题不在于能不能找到好点，而是好点出现之后传不下去。
