...
这次我把前面想的人为扰动方向实验真的跑了一遍，
而且分成了两步来看：
先看离线方向是不是对，
再看这个方向能不能传到真实 SUMO 指标上。

我这次用的是 ``epoch 1`` 的 ``round3``。
这里文件里的 ``round3`` 还是从 0 开始数的，
也就是平时口头说的第 4 个 round。

我先用 ``sample_eval_ep1_round3.csv`` 按真实 ``balance_score`` 排了一次：
...
这里的 ``0.1`` 就是我这次先手动设的步长，
意思可以直接理解成：
先让原样本朝更好的样本靠 10%，
看这个小步扰动本身是不是会把结果往好的方向推。
...
先重新采一条共享轨迹，
然后在同一个初始网络状态下，
分别评估 ``parent / pos / neg`` 三组样本的真实 SUMO 结果。

我这次在线小实验的结果是：

.. code-block:: text

   正向扰动后，balance_score 比 parent 更好：3 / 5
   反向扰动后，balance_score 比 parent 更差：1 / 5
   正向扰动后，worst_dir_queue 比 parent 更好：3 / 5
   反向扰动后，worst_dir_queue 比 parent 更差：1 / 5
   正向扰动后，dir_spread 比 parent 更好：3 / 5
   反向扰动后，dir_spread 比 parent 更差：1 / 5

三组样本的均值是：

.. code-block:: text

   parent 平均 balance_score: 44.0033
   pos 平均 balance_score: 34.4701
   neg 平均 balance_score: 40.8895
...
不是只在离线距离上看起来变了，
一放到真实 SUMO 里也不是完全没反应。

第二个点是，
这个方向现在还不算特别稳。
有些 pair 里 ``pos`` 确实比 ``parent`` 好，
但也有一些 pair 里 ``neg`` 反而更好，
说明我现在这版“朝单个最好样本靠近”的方向设计还比较粗。

另外我还注意到，
``pair 2`` 和 ``pair 4`` 里，
``parent`` 自己就很差，
而 ``pos`` 和 ``neg`` 都比 ``parent`` 好很多。
这让我觉得还有一种可能：
...
.. code-block:: text
   ...
   1. reward sequence 空间里确实存在一个可以人为控制的改进方向；
   2. 这个方向不只是离线有意义，已经能反映到真实 SUMO 指标上；
   3. 但当前“朝单个最好样本靠近”的人工方向还不够稳；
   4. 这也说明我原本的自动变异还没有稳定利用这个局部好方向。

换句话说，
我现在不会把这次结果理解成“整个框架已经完全没问题”，
而是会理解成：
整体框架的大方向没有偏，
但当前自动变异的方向性还不够强，
