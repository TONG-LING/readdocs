...
正向扰动后离好样本中心近了约 5.4%，离差样本中心远了约 5.0%，
两个方向变化幅度相近、方向相反，说明扰动并非随机漂移，
而是确实沿着有意义的方向移动了样本。

同时也检查了时间连续性：

一阶差分为相邻时间步 reward 的差值：:math:`\text{diff}[t] = \tau[t+1] - \tau[t]`。
一阶差分均值越小，说明 reward 序列在时间上越平滑。
此处记录的是正向扰动后一阶差分均值相对于原样本的变化量，
负值表示扰动使序列更平滑。
...
其中 ``worst_dir_queue`` 是所有方向中最长队列长度，衡量最堵方向的严重程度；
``dir_spread`` 是最长与最短队列的差值，衡量方向间的不均衡程度。
两者越低代表交通状况越好。
...
后续的目标是：
如果人工方向已经能在一些样本上带来改进，
那当前原有的变异在同样条件下为什么没有稳定做到向着好样本变异。
